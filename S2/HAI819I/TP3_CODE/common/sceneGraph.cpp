...
void SceneGraphLeaf::compute(Transform *t) {
        gameObject->compute(t);
}
...
void SceneGraphComposite:: compute(Transform *t) {
        
        gameObject->compute(t);
        Transform *tlocal= gameObject->transformation;
        for( auto &child : children ) {
            child->compute(tlocal);
        }
}
