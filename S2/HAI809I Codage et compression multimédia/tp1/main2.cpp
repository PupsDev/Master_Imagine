...
int main(int argc, char* argv[])
{
  ...
  if (argc != 5) 
   ...
   couleur *couleurArr = (couleur*)malloc(256 * sizeof(couleur));
   couleur *moyenArr = (couleur*)malloc(256 * sizeof(couleur));
   int *sum = (int*)malloc(256 * sizeof(int));
   ...
   {
     int p = rand()%nTaille3;
     couleurArr[i].r =ImgIn[p];
     couleurArr[i].g =ImgIn[p+1];
     couleurArr[i].b =ImgIn[p+2];
     moyenArr[i]=0;
     sum[i]=0;
   }
...
for(int it =0; it<10;it++)
{
  ...
  for (int i=0; i < nTaille3; i+=3)
  {
    ...
    c.g = ImgIn[i+1];
    c.b = ImgIn[i+2];
    k = nearestClust(couleurArr, c);
    ImgOut[i]=couleurArr[k].r;
    ImgOut[i+1]=couleurArr[k].g; 
    ImgOut[i+2]=couleurArr[k].b;
