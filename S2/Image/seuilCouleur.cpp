  ...
  */
  int nH, nW, nTaille;
   ...
   for (int j=0; j < nWT; j+=3)
     {
       if ( ImgIn[i*nW+j+composante] < S[0]) ImgOut[i*nW+j+composante]=0; 
       else if ( ImgIn[i*nW+j+composante] < S[1]) ImgOut[i*nW+j+composante]=128; 
       else ImgOut[i*nW+j+composante]=255;
