...
Sphere::Sphere(mlm::vec4 position, float radius)
{
    this->position = position;
    this->radius = radius;
}
Sphere::Sphere(mlm::vec4 position, float radius, Material* material)
{
    this->position = position;
    this->radius = radius;
...
bool Sphere::intersect(const Ray& r, float t_min, float t_max, Intersection& intersection) const
{
    ...
    mlm::vec4 L = r.getPosition() - this->position; 
...
bool Box::intersect(const Ray& r, float t_min, float t_max, Intersection& intersection)const
{
    ...
    intersection.hitPoint = r.at(intersection.t);
    intersection.normal = mlm::vec4(0.,0.,1.,0.);
    intersection.material = new Material(mlm::vec3(1.,1.,1.));
    return true; 
