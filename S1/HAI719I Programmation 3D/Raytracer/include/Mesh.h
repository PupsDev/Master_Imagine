...
class Mesh : public HittableObject
{
    public:
        Mesh();
        Mesh(std::vector<mlm::vec4> vertices, std::vector<size_t> indices,std::vector<mlm::vec4> normals);
        virtual bool intersect(const Ray& r, float t_min, float t_max, Intersection& intersection) const;
        virtual mlm::vec4 vertexAt(size_t indice)const;
        void setMaterial(Material* material){this->meshMaterial = material;}
        void setMaterialTriangle(size_t indice, Material * material);
        void apply(mlm::mat4 transformation);
        void flatShaded();
        void buildTriangles();
        void getBounds(mlm::vec4& vmin,mlm::vec4& vmax)
        {
            mlm::vec4 nmin = mlm::vec4(FLT_MAX,FLT_MAX,FLT_MAX,1.);
            mlm::vec4 nmax = mlm::vec4(-FLT_MAX,-FLT_MAX,-FLT_MAX,1.);
            float xmin = FLT_MAX;
            float ymin = FLT_MAX;
            float zmin = FLT_MAX;
            ...
            vmin =  mlm::vec4(xmin,ymin,zmin,1.);;
            vmax =  mlm::vec4(xmax,ymax,zmax,1.);;
    ...
    private:
    mlm::mat4 transformation;
    std::vector<MeshTriangle*> triangles;
    Material * meshMaterial;
    std::vector<mlm::vec4> normals;
    std::vector<mlm::vec4> verticesNormals;
