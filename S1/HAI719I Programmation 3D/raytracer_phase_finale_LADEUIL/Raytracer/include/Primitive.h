...
class Sphere : public Primitive
{
    public:
        Sphere();
        Sphere(float radius);
        Sphere(mlm::vec4 position,float radius);
        Sphere(mlm::vec4 position, float radius, Material* material);
        ...
        void getBounds(mlm::vec4& vmin,mlm::vec4& vmax)
        {
            ...
            vmin = this->position-mlm::vec4(size,size,size,0.);
            vmax = this->position+mlm::vec4(size,size,size,0.);
...
class Box : public Primitive
{
    public:
        Box();
        Box(mlm::vec4 vmin, mlm::vec4 vmax)
        {
            this->bounds[0] = vmin;
            this->bounds[1] = vmax;
