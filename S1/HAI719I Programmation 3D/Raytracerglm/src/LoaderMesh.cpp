...
LoaderObj::LoaderObj(std::string filename)
{
    ...
    std::vector<glm::vec4> normalsByIndice;
    size_t faceCount = 0;
    ...
    for( size_t i = 0 ; i < text.size(); i++)
    {
        switch(text[i][0])
        {
            ...
            case 'v':
                ...
                {
                    case 'n': // normals
                        normals.push_back( glm::vec4(tokensFloat[0],tokensFloat[1],tokensFloat[2],1.));
                    break;
                    case 't': // textures
                        textures.push_back( glm::vec4(tokensFloat[0],tokensFloat[1],1.,1.));
                    break;
                    default: // 'v' -> vertices
                        vertices.push_back( glm::vec4(tokensFloat[0],tokensFloat[1],tokensFloat[2],1.));
                    break;
                }
