...
struct TriangleVArray {
    ...
    unsigned int texMetal;
    unsigned int texNormal;
    unsigned int texRough;
    ...
    void loadTexture()
    {
        ...
        unsigned char *data = stbi_load("rust/rustediron2_basecolor.png", &width, &height, &nrChannels, 0);
     ...
     void loadTexture2(unsigned int & texture, char * path, int depth)
    {
        GLenum err;
        // texture 1
        // ---------
    ...
    void loadTextures()
    {
        ...
        loadTexture2(texColor, (char *)"rust/rustediron2_basecolor.png",32);
        loadTexture2(texMetal, (char *)"rust/rustediron2_metallic.png",8);
        loadTexture2(texNormal, (char *)"rust/rustediron2_normal.png",24);
        loadTexture2(texRough, (char *)"rust/rustediron2_roughness.png",8);
...
struct MeshVAO {
    ...
    void loadTexture()
    {
        ...
        unsigned char *data = stbi_load("rustediron2_basecolor.png", &width, &height, &nrChannels, 0);
        if (data)
        {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
            glGenerateMipmap(GL_TEXTURE_2D);
