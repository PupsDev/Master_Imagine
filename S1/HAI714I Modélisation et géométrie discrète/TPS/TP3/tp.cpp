...
void draw (std::vector<Vec3> & o_vertices) {
...
std::vector<Vec3> BezierCurveByCasteljau(std::vector<Vec3> controlPoints, int nbU)
{
     std::vector<Vec3> listePoints;
    ...
    for(long i = 0 ; i<nbU; i++)
    {
        ...
        for(size_t k =0 ; k < controlPoints.size();k++)
        {
            point = doublePoint(controlPoints[k],controlPoints[k+1],u);
        }
         
        point[2]=0.;
        //point=point/controlPoints.size();
        listePoints.push_back(point);
    }
    
    listePoints.push_back(controlPoints[controlPoints.size()-1]);
...
int main (int argc, char ** argv) {
    ...
    courbe.resize(nbU);
    /*
    courbe = HermiteCubicCurve( Vec3(0,0,0), Vec3(2,0,0), Vec3(1,1,0), Vec3(1,-1,0), nbU);
*/
    courbe[0]=Vec3(0,0,0);
    courbe[1]=Vec3(0.5,1,0);
    courbe[2]=Vec3(1.5,1,0);
    courbe[3]=Vec3(2,0,0);


    courbe2.clear();
    courbe2.resize(nbU2);
    courbe2 = BezierCurveByBernstein( courbe, nbU2);
