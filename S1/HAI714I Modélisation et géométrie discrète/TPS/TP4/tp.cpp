...
std::vector< Vec3 > courbeH;

std::vector< Vec3 > courbe2;
std::vector< Vec3 > courbe3;
std::vector< Vec3 > courbe4;
std::vector< Vec3 > courbe5;

std::vector <std::vector< Vec3 >> courbes;
std::vector <std::vector< Vec3 >> courbes2;
Basis basis;
...
void draw (std::vector<Vec3> & o_vertices, Vec3 color) {
...
void drawSurface () {
    ...
    for(size_t i = 0 ; i <courbes.size()-2;i++)
    {
        for(size_t j = 0 ; j < courbes[i].size()-2; j++)
        {
            glBegin(GL_TRIANGLES);
            glColor3f( 0.8, 0.8, 0.8 );

            glVertex3f(courbes[i][j+1][0], courbes[i][j+1][1], courbes[i][j+1][2]);
            glVertex3f(courbes[i+1][j][0], courbes[i+1][j][1], courbes[i+1][j][2]);
            glVertex3f(courbes[i][j][0], courbes[i][j][1], courbes[i][j][2]);
            ...
            glVertex3f(courbes[i+1][j+1][0], courbes[i+1][j+1][1], courbes[i+1][j+1][2]);
            glVertex3f(courbes[i+1][j][0], courbes[i+1][j][1], courbes[i+1][j][2]);
            glVertex3f(courbes[i][j+1][0], courbes[i][j+1][1], courbes[i][j+1][2]);
...
void display () {
    ...
    // Même couleur que les points de controles 
    colors.push_back(Red+Green+Blue);

    // 5 Couleurs pour 5 itérations de de casteljau 
    colors.push_back(Red);
    colors.push_back(Red+Green);
    colors.push_back(Blue);
    colors.push_back(Red+Blue);
    colors.push_back(Green+Blue);

    // Un joli vert pour le tracé de la courbe en colors[6]
    colors.push_back( (46./255.)*Red+(113./255.)*Blue+(204./255.)*Green);


    draw (courbe, Red+Green+Blue);
    draw (courbeH, Red+Green+Blue);
    draw (courbe2, Vec3(0.0,0.0,0.0));
...
float polynomeBernstein(float u, int n, int i)
{
    float combi = factorial(n)/ (factorial(i)*(factorial( (n-i))));
    return combi*pow(u,i)*pow((1-u),n-i);
}

std::vector<Vec3> BezierCurveByBernstein(std::vector<Vec3> controlPoints, int nbU)
{
    ...
    for(long i = 0; i<nbU;i++)
    {
        ...
        for(size_t j =0 ; j< 3;j++)
        {
            ...
            {
                float Bi = polynomeBernstein(u,controlPoints.size()-1,k);
                point[j]+= (Bi*controlPoints[k][j]);
...
std::vector<Vec3> Casteljau(std::vector<Vec3> controlPoints, float u)
{
    std::vector<Vec3> listePoints;

    for(size_t k =0 ; k < (controlPoints.size()-1);k++)
    {
        listePoints.push_back(doublePoint(controlPoints[k],controlPoints[k+1],u));
    }
...
void DrawBezierCurveByCasteljau(std::vector<Vec3> controlPoints, int nbU)
{
    std::vector<Vec3> listePoints;
    size_t taille = controlPoints.size();
    
    // itération sur le nombre de points
    for(size_t j = 0 ; j < (size_t)nbU+1; j++)
    {
        float u = (float)(j)/(float)nbU;
        listePoints = controlPoints;

        // Itération sur les points de controles
        for(size_t i = 0 ; i < taille-1 ; i++)
        {
            listePoints = Casteljau( listePoints,u);
        }

        // A la fin il ne reste qu'un seul point qui appartient à la courbe 
        courbes[6].push_back(listePoints[0]);
    }


}

void DrawCasteljauAtJ(int j , int nbU)
{
    courbes[0].resize(courbe.size());
    courbes[0] = courbe;
    float u = (float)(j)/(float)nbU;
    for(size_t i = 0 ; i < courbe.size()-1 ; i++)
    {
        courbes[i+1] = Casteljau( courbes[i],u);
    }
}
void DrawBezierCurveByBernstein(std::vector<Vec3> controlPoints, int nbU)
{
    courbe2.clear();
    courbe2.resize(nbU);
    courbe2 = BezierCurveByBernstein( controlPoints, nbU);

}

// u sur la droite
// v sur la génératrice controlPoints

std::vector<std::vector<Vec3>> SurfaceCylindrique(std::vector<Vec3> controlPoints, Vec3 droiteOrigine, Vec3 droiteDirection, int u, int v)
{
    std::vector<std::vector<Vec3>> surface;
    for(size_t j = 0 ; j < controlPoints.size() ;j++)
    {
        controlPoints[j]+= droiteOrigine;
    }
    for(int i = 0 ; i < u ;i++)
    {
        for(size_t j = 0 ; j < controlPoints.size()+1 ;j++)
        {
            controlPoints[j]= controlPoints[j]+droiteDirection/u;
        }
        std::vector<Vec3> generatrice = BezierCurveByBernstein(controlPoints, v);
        surface.push_back(generatrice);
...
std::vector<std::vector<Vec3>> SurfaceRegle(std::vector<Vec3> controlPointsU, std::vector<Vec3> controlPointsV, int nbu, int nbv)
{
    std::vector<std::vector<Vec3>> surface;
    ...
    {
        float u = (float)i/(float)nbu;
        std::vector<Vec3> generatrice;
        for(int j = 0 ; j < nbv;j++)
        {
            float v = (float)j/(float)nbv;
            Vec3 point=Vec3(0.,0.,0.);
            for(size_t j =0 ; j< 3;j++)
            {
                for(size_t k=0;k<controlPointsU.size();k++)
                {
                    float Bi = polynomeBernstein(u,controlPointsU.size()-1,k);
                    point[j]+= (1-v)*(Bi*controlPointsU[k][j]);

                }
                for(size_t k=0;k<controlPointsV.size();k++)
                {
                    float Bi = polynomeBernstein(u,controlPointsV.size()-1,k);
                    point[j]+= v*(Bi*controlPointsV[k][j]);

                }
                    
            }
            generatrice.push_back(point);
        }
        surface.push_back(generatrice);
...
std::vector<std::vector<Vec3>> SurfaceGrid(std::vector<std::vector<Vec3>> controlGridPoints,  int nbu, int nbv)
{
    std::vector<std::vector<Vec3>> surface;
    
    for(int i = 0 ; i < controlGridPoints.size() ;i++)
    {
        //float u = (float)i/(float)nbu;
        std::vector<Vec3> generatrice;
        std::vector<Vec3> generatrice2;

        generatrice = BezierCurveByBernstein(controlGridPoints[i], nbu);
        generatrice2 = BezierCurveByBernstein(controlGridPoints[i+1], nbu);
        SurfaceRegle(std::vector<Vec3> controlPointsU, std::vector<Vec3> controlPointsV, int nbu, int nbv)
        surface.push_back(generatrice);
...
int main (int argc, char ** argv) {
    ...
    // Soit on dessine la courbe de Hermite
    //courbes.resize(10);
    courbe.clear();
    courbe.resize(nbU);
    courbeH.clear();
    ...
    // A droite de l'écran
    courbeH = HermiteCubicCurve( Vec3(0,0,0), Vec3(2,0,0), Vec3(1,1,0), Vec3(1,-1,0), nbU);
    

    // Soit la courbe est redéfini ici
    // Points de controles
    courbe[0]=Vec3(0-2.1,0-1,0);
    courbe[1]=Vec3(0.5-2.1,1-1,0);
    courbe[2]=Vec3(1.5-2.1,1.5-1,0);
    courbe[3]=Vec3(1.75-2.1,1.5-1,0);
    courbe[4]=Vec3(2.5-2.1,0-1,0);

    courbeH[0]=Vec3(0-2.1,0-2,1.);
    courbeH[1]=Vec3(0.5-2.1,1-1,1.);
    courbeH[2]=Vec3(1.5-3.1,1.5-1,1.);
    courbeH[3]=Vec3(1.75-2.1,1.5-1,1.);
    courbeH[4]=Vec3(2.5-2.1,0-3,1.);

     std::vector<std::vector<Vec3>> grid;
    grid.push_back(courbe);
    grid.push_back(courbeH);
/*
    // On dessine la courbe de bézier à gauche de l'écran soit par l'algo de Bernstein soit par celui de De Casteljau, elles sont superposées

    // En noir
    DrawBezierCurveByBernstein(courbe, nbU2);
    
    // En vert
    DrawBezierCurveByCasteljau(courbe, nbU2);

    // La construction de casteljau au point j = nbU2/2
    DrawCasteljauAtJ( nbU2/2 ,  nbU2);
*/
    std::vector<std::vector<Vec3>> surface;
    
    long u = 50;
    long v = 20;
    surface = SurfaceCylindrique(courbe, Vec3(0.,0.,0.), Vec3(0.,0.,-1.), u, v);
    surface = SurfaceGrid(grid, u, v);
    ...
    {
        courbes[i].resize(surface[i].size());
        courbes[i]=surface[i];
    }
    /*
