...
struct Mesh {
    ...
    void computeTrianglesNormals(){
        ...
        for(size_t i = 0 ; i < triangles.size(); i++)
        {
        	...
        	Vec3 normale = normale.cross(e_10,e_20);
        	normale.normalize();
        	triangle_normals.push_back(normale);
        }
    }	
    ...
    void computeVerticesNormals(){
        ...
        // normals.clear() produit un segmentation fault ?
        //normals.clear();
        ...
        for(size_t i = 0 ; i < triangles.size(); i++)
        {
        	Vec3 triangle_normal = triangle_normals[i];
        	
        		normals[triangles[i][0]]+=triangle_normal;
        		normals[triangles[i][1]]+=triangle_normal;
        		normals[triangles[i][2]]+=triangle_normal;
...
int main (int argc, char ** argv) {
    ...
    mesh_transformation.rotation = Mat3(
    										0., -1., 0.,
    										1., 0., 0.,
    										0., 0., 1.
    									);
    mesh_transformation.rotation=mesh_transformation.rotation*scale;
    mesh_transformation.translation = Vec3( 0.5, 0.5, 0.5 );
